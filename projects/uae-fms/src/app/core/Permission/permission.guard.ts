...
export class PermissionGuard implements CanActivate {
  constructor(private _router: Router , private _facadeProfile: UserProfileFacade){}
  canActivate(
    ...
    if (userRoles && permission) {
      for (const checkPermission of permission) {
        const permissionFound = userRoles.find(x => x.toUpperCase() === checkPermission.toUpperCase());
