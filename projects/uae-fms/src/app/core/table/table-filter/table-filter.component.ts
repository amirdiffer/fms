...
export class TableFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.getSavedDataFilter().length > 0 && this.entity) {
      ...
      columns = (<Array<object>>columns).filter(x => x['name'] == this.entity)[0]['value'];
      this.items = (<Array<object>>columns).map(x => {
        return {
          lable: x['lable'],
          name: x['name'],
          value: ''
  ...
  updateColumn(e?) {
    ...
    this.customizeFilter = this.allColumns.map(x => {
      return {
        lable: x.lable,
        name: x.name,
        value: ''
  ...
  saveFilter() {
    if (this.entity) {
      if (this.getSavedDataFilter().length > 0) {
        ...
        let withoutEntity = (<Array<object>>this.getSavedDataFilter()).filter(x => x['name'] != this.entity);
