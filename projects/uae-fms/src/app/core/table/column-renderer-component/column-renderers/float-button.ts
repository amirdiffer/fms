    ...
    >
      <ng-container *ngFor="let item of setting?.floatButton">
        <ng-container *ngIf="checkPermission(item)">
          <span
            *ngIf="checkCondition(item)"
            (click)="clicked(item, col, data)"
            (mouseenter)="item.tooltip ? tooltipEnter() : null"
            (mouseleave)="item.tooltip ? tooltipLeave() : null"
          >
            <p
              *ngIf="item.button == 'folder-check' && item.tooltip"
              class="tooltip-float-button"
              #tooltip
            >
              {{ item.tooltip }}
            </p>
            <svg-icon
              *ngIf="item.button !== 'receive'"
              ...
              [svgStyle]="{
                ...
                'width.em': item.button == 'folder-check' ? 2 : 1.8
              }"
            ></svg-icon>
            <img
              *ngIf="item.button === 'receive'"
    ...
    this._facadeProfile.loadData$.subscribe((x) => {
      if (x && x.roles[0].permissions && setting.permission) {
        for (const checkPermission of setting.permission) {
          const permissionFound = x.roles[0].permissions.find(
