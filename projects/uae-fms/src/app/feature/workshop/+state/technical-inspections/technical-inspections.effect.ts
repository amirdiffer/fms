...
export class TechnicalInspectionEffect {
  constructor(private action$: Actions, private service: TechnicalInspectionService) { }

  loadAll$ = createEffect(() =>
    this.action$.pipe(
      ofType(TechnicalInspectionActions.loadAll),
      mergeMap(
        action => this.service.loadAll().pipe(
          map(
            data => {
              return TechnicalInspectionActions.allDataLoaded({ data });
            }
          ),
          catchError(error => of(TechnicalInspectionActions.error({ reason: error })))
        )
      )
    )
  );
