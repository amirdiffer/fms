...
import { initialState, technicalInspectionAdapter , ITechnicalInspectionState } from "./technical-inspections.entity";
...
const technicalInspectionReducer = createReducer(
    initialState,
    on(
        TechnicalInspectionActions.loadAll,
        (state) => ({...state, loaded:false, error:null , message: null})
    ),
    on(
        TechnicalInspectionActions.allDataLoaded,
        (state, { data }) => ( technicalInspectionAdapter.setAll(data, {...state, loaded:true, error:null}))
    ),
    on(
        TechnicalInspectionActions.error,
