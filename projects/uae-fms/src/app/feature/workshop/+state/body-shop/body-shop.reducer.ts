...
import { initialState, bodyShopAdapter, BodyShopState } from "./body-shop.entity";
...
const bodyShopReducer = createReducer(
  initialState,
  on(
    BodyShopActions.loadAll,
    (state) => ({ ...state, loaded: false, error: null, message: null })
  ),
  on(
    BodyShopActions.allDataLoaded,
    (state, { data }) => (bodyShopAdapter.setAll(data, { ...state, loaded: true, error: null }))
  ),
  on(
    BodyShopActions.error,
