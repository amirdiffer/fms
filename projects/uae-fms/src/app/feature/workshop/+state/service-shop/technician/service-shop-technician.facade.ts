...
export class ServiceShopTechnicianFacade {
  serviceShop$ = this.store.pipe(
    select(ServiceShopTechnicianSelectors.selectAll)
  );
  ...
  submitted$ = this.store.pipe(
    select(ServiceShopTechnicianSelectors.submitted)
  );

  conut$ = this.store.pipe(select(ServiceShopTechnicianSelectors.count));

  constructor(private store: Store<IServiceShopTechnicianPartialState>) {this.loadAll()}
