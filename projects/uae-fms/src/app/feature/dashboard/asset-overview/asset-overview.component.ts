...
export class AssetOverviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._assetOverview.overviewAsset().subscribe(x => {
      ...
      this.bussinessCategoryChartData = this.overviewData.numOfAssetsByBusinessCategory.map((d, i) => {
      ...
      this.AssetDepartmentChartData = this.overviewData.numOfAssetsByOrganization.map((d, i) => {
        let color = i==0 ? '#39DA8A' : i==1 ? '#6F7BF0': '#FF5B5C'
        return {
    ...
    this._accessoryOverview.accessoryOverview().subscribe((x) => {
      let data = x.message;
      this.accessoryOverviewData = [data.numOfAssignedAccessories, data.numOfInactiveAccessories]
    });
    this._subassetOverview.subAssetOverview().subscribe(x => {
      let data = x.message;
      this.subassetOverviewData = [data.numOfActiveSubAssets, data.numOfInactiveSubAssets, data.numOfXFleetSubAssets]
    })
  }
