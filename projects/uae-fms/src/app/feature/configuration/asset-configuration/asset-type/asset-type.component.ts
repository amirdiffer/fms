...
export class AssetTypeComponent implements OnInit, OnDestroy {
  ...
  createMake(assetType: AssetTypeExtension): void {
    ...
    this.router
      .navigate([this.activeAssetType + '/add-make/' + assetType.id], {
  ...
  createModel(assetTypeId, make: MakeExtension): void {
    ...
    this.router.navigate(
      [`${this.activeAssetType}/add-model/${assetTypeId}/${make.id}`],
      { relativeTo: this.activatedRoute }
    );
  }
  ...
  createTrim(assetTypeId, makeId, model): void {
    ...
    this.router.navigate(
      [`${this.activeAssetType}/add-trim/${assetTypeId}/${makeId}/${model.id}`],
      { relativeTo: this.activatedRoute }
    );
  }
  ...
  onTypeClick(item: AssetTypeExtension): void {
    ...
    this.selectedMake = -1;
    this.assetType$
      .forEach((assetType: AssetTypeExtension[]) => {
  ...
  changeType(type: string, refresh: boolean = false) {
    ...
    if (type == 'ASSET' || type == 'SUB_ASSET') {
      this.assetType$ = (type == 'ASSET'
        ? this._assetTypefacade.assetType$
        : this._subAssetTypeFacade.subAssetType$
      ).pipe(
        map((response) => {
          assetTypeArray = [];
  ...
  getAssetText() {
    if (this.activeAssetType == 'ASSET')
      return 'configuration.asset_configuration.add_asset_type';
    if (this.activeAssetType == 'SUB_ASSET')
      return 'configuration.asset_configuration.add_sub_asset_type';
    if (this.activeAssetType == 'ACCESSORY')
      return 'configuration.asset_configuration.add_accessory_type';
  }

  addCategoryType() {
    switch (this.activeAssetType) {
      case 'ASSET':
        this.router.navigate(['add-asset-configuration'], {
          relativeTo: this.activatedRoute
        });
        break;

      case 'SUB_ASSET':
        this.router.navigate(['add-sub-asset-configuration'], {
          relativeTo: this.activatedRoute
        });
        break;

      case 'ACCESSORY':
        this.router.navigate(['add-accessory-configuration'], {
          relativeTo: this.activatedRoute
        });
        break;
    }
  ...
  editCategoryType(asset) {
    ...
    switch (this.activeAssetType) {
      case 'ASSET':
        this.router.navigate(['edit-asset-configuration/' + asset.id], {
          relativeTo: this.activatedRoute
        });
        break;

      case 'SUB_ASSET':
        this.router.navigate(['edit-sub-asset-configuration/' + asset.id], {
          relativeTo: this.activatedRoute
        });
        break;

      case 'ACCESSORY':
        this.router.navigate(['edit-accessory-configuration/' + asset.id], {
          relativeTo: this.activatedRoute
        });
        break;
    }
  ...
  editMake(make, assetType) {
    this.router
      .navigate(
        [`${this.activeAssetType}/edit-make/${assetType.id}/${make.id}`],
        { relativeTo: this.activatedRoute }
      )
      .then(() => {
        this.onTypeClick(assetType);
  ...
  editModel(model, makeId, assetTypeId) {
    this.router
      .navigate(
        [
          `${this.activeAssetType}/edit-model/${assetTypeId}/${makeId}/${model.id}`
        ],
        { relativeTo: this.activatedRoute }
      )
      .then(() => {
  ...
  refreshData() {
    /* Refresh */
    this.activatedRoute.children.map((x) => {
      ...
      this.assetTypeSubject$.subscribe((y) => {
        if (y && y.length > 0) {
          ...
          if (assetTypeId && !type) {
            type = y.find((z) => z.id == assetTypeId)
              ? y.find((z) => z.id == assetTypeId)
              : null;
            type != null ? this.onTypeClick(type) : null;
          }
          if (assetTypeId && (makeId || id) && !make) {
            let makes = y.find((z) => z.id == assetTypeId)
              ? y.find((z) => z.id == assetTypeId).makes
              : null;
            makes != null
              ? ((make = makes.find(
                  (make) => make.id == (makeId ? makeId : id)
                )),
                this.onMakeClick(make, assetTypeId))
              : null;
          }
          if (assetTypeId && makeId && (modelId || id) && !model) {
            make
              ? ((model = make.models.find(
                  (model) => model.id == (modelId ? modelId : id)
                )),
                this.onModelClick(model, makeId, assetTypeId))
              : null;
          }
