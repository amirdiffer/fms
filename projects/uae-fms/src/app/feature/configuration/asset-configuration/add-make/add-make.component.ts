...
export class AddMakeComponent
  ...
  ngOnInit(): void {
    ...
    }else if (activeRoute.find(item => item == "sub-asset-edit-make")){
    ...
    if(this.isEditing && this.assetTypeId  && id){
      console.log(this.assetTypeId  , id , this.selectedType)
      if(this.selectedType === 'SUB_ASSET'){
        this._subAssetTypeFacade.specificSubAssetType$.subscribe(x => {
          if(x){
            let makes = {
              name : x.makes.find( y => y.id == id).name,
              description : x.makes.find( y => y.id == id).description,
            }
            console.log(makes)
