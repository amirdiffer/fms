...
import { initialState , IUserProfileState, userProfileAdapter } from "./user.entity";

const userProfileReducer = createReducer (
    initialState,
    on(UserPorfileAction.loadData ,
        (state) => ({
            ...state, loaded: false , error: null , message:null
        })
    ),
    ...
    on(UserPorfileAction.error, (state, { reason }) => ({
        ...state, error: reason,loaded: true})
    )
